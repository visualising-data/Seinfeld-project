<script>
  ...
  let { currentSection, selectors, activeSelector, handleCharacterClick } = $props();
</script>
...
<ul class="grid gap-6 flex-wrap {currentSection === 'main_chars' ? 'grid-cols-1' : 'grid-cols-2'}">
  {#each selectors as selector}
    <li class="flex justify-center">
      <button 
        class="selector-button flex flex-col items-center max-w-28 {activeSelector === selector.id ? 'active' : ''}"
        onclick={() => handleCharacterClick(selector.id)}>
        <div class="selector rounded-full bg-contain bg-center opacity-50" 
            style="background-image: url('{currentSection === "locations" ? getLocationIconPath(selector.id) : getCharacterImagePath(selector.id)}'); width: 75px; height: 75px;"></div>
        <div class="text-center pt-1" style="font-size: 1.25rem; line-height: 1.2;">{selector.label}</div>
      </button>
    </li>
  {/each}
</ul>
